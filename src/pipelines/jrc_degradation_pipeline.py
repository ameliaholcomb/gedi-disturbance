...
def run_main(shots_dir=None, out_dir=None):
    ...
    degrade_shards_df = spark.createDataFrame(jrc_parser.get_sharding_geoms())
    ...
    shots_df = degradation_overlap.run_degradation_overlay(
        spark=spark,
        shots_df=shots_df,
        in_schema=jrc_parser.in_schema,
        out_schema=jrc_parser.out_schema,
        degradation_overlay_fn=jrc_parser.get_degradation_event_dates_for_shot_pair_no_deforestation,
    )

    shots_df.filter("measured_disturbance + control_disturbance > 0").write.mode(
        "overwrite"
    ).format("parquet").save(out_dir.as_posix())


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Find GEDI shot pairs with interceding degradation, as detected by the JRC product."
    )
    parser.add_argument(
        "--shots_dir",
        "-s",
